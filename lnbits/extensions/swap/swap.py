...
async def create_reverse_swap(swap_id, data: CreateReverseSubmarineSwap):
    ...
    # print('base/quote: ' + data.base + '/' + data.quote)
    if '⚡' in data.base:
        data.base = 'BTC'
    if '⚡' in data.quote:
        data.quote = 'BTC'
    ...
    amount_msat = data.amount * 1000 * 10**8
    ...
    asyncio.ensure_future(wait_for_onchain_tx(swap, res["minerFeeInvoice"]))
    ...
    while swap.status != 'completed' and swap.status != 'transaction.failed' and swap.status != 'swap.expired':
        ...
        if swap_status['status'] == 'swap.created':
            ...
            task = asyncio.ensure_future(pay_invoice(
                ...
                description=f"reverse submarine swap for {swap.amount} sats on marduk.exchange",
                extra={"tag": "swap", "swap_id": swap.id, "reverse": True},
            ))
        else:
...
#         # pseudozach: its not a deadlock - its an hold invoice
#         task = asyncio.ensure_future(pay_invoice(
#            wallet_id=swap.wallet,
#            payment_request=invoice,
#            description=f"reverse submarine swap for {swap.amount} sats on marduk.exchange",
